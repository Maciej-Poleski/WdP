...
# 48 "main.c" 2
# 1 "/usr/lib/gcc/x86_64-pc-linux-gnu/4.6.2/include/stdbool.h" 1 3 4
# 49 "main.c" 2
...
_Bool count(int row)
{
    ...
    *xPtr++=row;
    ...
    for(i=0;i<m;++i)
    {
        if(x[row] || y[i] || xpy[row+i] || xmy[row-i+m-1])
            continue;
        x[row]=y[i]=xpy[row+i]=xmy[row-i+m-1]=1;
        *yPtr++=i;
        if(count(row+1))
            return 1;
        --yPtr;
        x[row]=y[i]=xpy[row+i]=xmy[row-i+m-1]=0;
    }
    ++s;
    --xPtr;
    return 0;
...
inline void solution(void)
{
    ...
    while(z--)
    {
        scanf("%i%i%i",&n,&m,&s);
        storeS=s;
        memset(x,0,sizeof(_Bool)*n);
        memset(y,0,sizeof(_Bool)*m);
        memset(xpy,0,sizeof(_Bool)*(n+m-1));
        memset(xmy,0,sizeof(_Bool)*(n+m-1));
        if(count(0))
        {
            printf("TAK\n");
            checkStack();
